...
var ListaEstudioInversion = function () {
    ...
    var myTableUA = $("#dynamic-table-EstudioInversion").DataTable({
        ...
        "columns": [
            {
                "data": function (data) {
                    return '<a data-toggle="tooltip" title="ver etapa"><button onclick="cargarDatosEstudioInversion(' + data.codigo_unico_est_inv + ')" data-toggle="tooltip" title="ver etapa" type="button" class="btn btn btn-warning btn-xs">' + data.codigo_unico_est_inv + ' </button></a>';
                }
            },
            ...
            {
                "data": function (data, type, dataToSet) {
                    return 'S/' + parseFloat(data.monto_inv).toLocaleString('en-US');
                }
            },
            {
                "data": function (data, type, dataToSet) {
                    return 'S/' + parseFloat(data.costo_estudio).toLocaleString('en-US');
                }
            },
            {
                "data": function (data, type, dataToSet) {
            ...
            { "data": "ultimoEstudio" },
            { "defaultContent": "  <button type='button' title='Repositorio' class='repositorio btn btn-info btn-xs' data-toggle='modal' data-target='#repositorio'><i class='fa fa-hdd-o' aria-hidden='true'></i> Repositorio</button>" }
        ],
        "language": idioma_espanol
    });
...
function cargarDatosEstudioInversion(codigoProyectoInversion) {
    
    $.ajax(
          {
              url:base_url+"index.php/bancoproyectos/getProyectoPIDE",
              type: 'POST',
              cache: false,
              data:{codigo:codigoProyectoInversion},
            ...
            function(dataA)
              {
                ...
                var data = dataA.data[0];
                var localizaciones_ = dataA.localizacion;
                ...
                if(Object.keys(data).length!=0)
                {
                  ...
                  var htmlBody='';
                  localizaciones_.forEach(element =>
                 {
