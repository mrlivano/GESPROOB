		...
		<br>
  		<table id="tablaPresentacion" style="width: 100%" border="1">
			...
			<?php foreach($listarPiePresupuestoAD as $item){ ?>
				<tr>
				<td style="<?=$item->gasto?'font-weight:bold;':'font-weight: bold; background-color:#f8f8f8;'?>"><?=$item->descripcion?></td>
				<td style="text-align:right; <?=$item->gasto?'font-weight:regular;':'font-weight: bold; background-color:#f8f8f8;'?>">S/ <?=a_number_format($item->monto, 2, '.',",",3)?></td>
			</tr>
			<?php } 
			?>
			<?php }
