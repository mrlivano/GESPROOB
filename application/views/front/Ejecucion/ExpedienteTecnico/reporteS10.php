				...
				<div class="x_content">
					<div class="row">
						...
						<div class="col-md-2 col-sm-2 col-xs-2 colPage1">
							...
							<div style="height: 536px;overflow: scroll; background-color: transparent;">
						...
						<div class="col-md-10 col-sm-10 col-xs-10 colPage2">
							...
							<div class="pull-right tableTools-containerr"></div>
								<table id="tableCostoUnitario" class="table table-striped jambo_table bulk_action  table-hover" cellspacing="0" width="100%">
									<thead>
										<tr>
											<th class="center" style="width: 55%">Descripcion</th>
											<th class="center" style="width: 5%">Und.</th>
									...
									<tbody id="bodyCostoUnitario"></tbody>
								</table>
							</div>
						</div>
					</div>
...
<script>
	...
	function costoUnitario(CodigoUnico, CodigoPresupuesto, CodigoSubPresupuesto, CodigoPartida, key, element) {
		...
		$.ajax({
			...
			success: function(resp) {
				...
				$('#bodyCostoUnitario').html(htmlTemp);
				toolsCostoUnitario();
			}
		});
	}

	function destroyHojaPresupuesto() {
		var table = $("#tableHojaPresupuesto").dataTable().fnDestroy();
	}

	function toolsHojaPresupuesto() {

		var myTable = $('#tableHojaPresupuesto').DataTable({
	...
	function destroyCostoUnitario() {
		var table = $("#tableCostoUnitario").dataTable().fnDestroy();
	}

	function toolsCostoUnitario() {

		var myTable = $('#tableCostoUnitario').DataTable({
			"language": idioma_espanol,
			"searching": true,
			"info": true,
			"paging": true,
